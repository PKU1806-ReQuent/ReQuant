#!/bin/bash
# Sequential runner for Qwen3-14B baselines (Quarot + W4A${A_BITS:-4}, with in-process lm_eval).
...
A_BITS=${A_BITS:-4}
LOG_DIR=${LOG_DIR:-logs_14b_w4a${A_BITS}}
...
# 1) GPTAQ + Quarot (W4A${A_BITS}), 8-GPU DP
run_step 01_gptaq_w4a${A_BITS}_rot \
    env MASTER_PORT=29621 \
    bash scripts/gptaq.sh "${MODEL}" "${GPUS_8}" 8

# 2) GPTQ + Quarot (W4A${A_BITS}), 8-GPU DP
run_step 02_gptq_w4a${A_BITS}_rot \
    env MASTER_PORT=29611 \
    bash scripts/gptq.sh "${MODEL}" "${GPUS_8}" 8

# 3) RTN + Quarot (W4A${A_BITS}), 8-GPU DP
run_step 03_rtn_w4a${A_BITS}_rot \
    env MASTER_PORT=29641 \
    bash scripts/rtn.sh "${MODEL}" "${GPUS_8}" 8

# 4) AWQ + Quarot (W4A${A_BITS}), 8-GPU DP
run_step 04_awq_w4a${A_BITS}_rot \
