...
#
# Included runs:
#   - Default active set: GPTAQ/GPTQ/RTN/AWQ with base, ReQuant,
#     QuaRot, and QuaRot + ReQuant variants.
#   - QuaRot means ROTATE=1. Default precision is W4A16.
#
# Usage:
#   bash scripts/run_w4a16_ablation.sh [model path] [GPU list] [nproc]
#
# Example:
#   bash scripts/run_w4a16_ablation.sh ./modelzoo/Llama2/Llama-2-7b-hf "3,4,5,6" 4

set -euo pipefail

MODEL_PATH=${1:-./modelzoo/Llama2/Llama-2-7b-hf}
GPU_LIST=${2:-"3,4,5,6"}
NPROC=${3:-4}
...
run_gpt_one() {
  ...
  if [[ "${method}" == "gptaq" ]]; then
    qmodel_path="./outputs/$(basename "${MODEL_PATH}")/${method}/${method}_w4${ACT_TAG}_ns${N_SAMPLES}_a${ALPHA}${rotate_tag}${requant_tag}.pt"
  else
    qmodel_path="./outputs/$(basename "${MODEL_PATH}")/${method}/${method}_w4${ACT_TAG}_ns${N_SAMPLES}${rotate_tag}${requant_tag}.pt"
  fi
  ...
  echo "[Run] ${name}"
  echo "[Config] model=${MODEL_PATH} GPUs=${GPU_LIST} nproc=${NPROC}"
  echo "[Config] method=${method} N_SAMPLES=${N_SAMPLES} SEQ_LEN=${SEQ_LEN} A_BITS=${A_BITS} ROTATE=${rotate} REQUANT=${requant} REQUANT_BETA=${method_beta} REQUANT_MIN_GAIN_EPS=${REQUANT_MIN_GAIN_EPS}"
  echo "[Config] qmodel=${qmodel_path}"
  echo "============================================================"
...
#   run_gpt_one <gptq|gptaq> "<name>" <rotate:0|1> <requant:0|1>
