# coding=utf-8
"""
GPTAQ (base) with data-parallel Hessian / dXXT accumulation (multi-process, multi-GPU).

Mirrors ``gptaq_refined_dp_utils.gptq_refined_fwrd_data_parallel`` but uses ``GPTAQ`` /
``FPInputsCache`` from ``gptaq_utils`` (no Phase-2 refinement).  ``GPTAQ.add_batch`` matches
``GPTAQRefined.add_batch`` (token-level ``tmp`` for 3D activations, optional ``fp_inp``).
Weighted merge of
per-rank ``H``, ``dXXT`` and optional ``--dp_fasterquant_master_only`` /
``--dp_shard_inps`` follow the same contract as the refined DP module.
"""
...
from gptq_utils.gptaq_utils import FPInputsCache, GPTAQ
...
def _merge_hessian_stats(
    ...
    """All-reduce merged H and dXXT (same as ``gptaq_refined_dp_utils``)."""
    ...
    if n_total < 1.0:
        raise RuntimeError(
            "gptaq_dp: total calibration token count N is zero; "
...
def _merge_hessian_stats_to_rank0(
    ...
    """Weighted merge to rank 0 only via ``reduce``."""
...
def gptq_fwrd_data_parallel(
    ...
    """
    Same outer flow as ``gptaq_utils.gptq_fwrd``, but ranks shard calibration indices
    for FP + quant hooks, then merge H/dXXT before ``fasterquant``.
    """
    if not dist.is_available() or not dist.is_initialized():
        raise RuntimeError(
            "gptaq_dp: gptq_fwrd_data_parallel requires torch.distributed; "
            "launch with torchrun (see ptq_gptaq_dp.py)."
        )
    ...
    logging.info(
        "-----GPTAQ DP (H/dXXT sharded) rank=%d world=%d local_samples=%d/%d "
    ...
    pbar = tqdm(
        range(len(layers)),
        ncols=120,
        desc="Quantizing Layers (GPTAQ DP)",
        disable=(rank != 0),
    )
    for i in pbar:
    ...
    logging.info("-----GPTAQ DP Quantization Done (rank=%d)-----\n", rank)
    return quantizers
