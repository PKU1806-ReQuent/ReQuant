# coding=utf-8
"""RTN weight quantization for the torchrun data-parallel PTQ entry."""
...
def _quantize_layer_weights(
    ...
    for names in sequential_names:
        for name in names:
            ...
            linear.weight.data = q_weight.to(linear.weight.data.dtype)
            quantizers[f"model.layers.{layer_idx}.{name}"] = quantizer.cpu()
    return quantizers


def _broadcast_parameters_contiguous(
...
def rtn_fwrd_data_parallel(
    ...
    logging.info(
        "-----RTN DP rank=%d world=%d requant=%s-----",
        rank,
        world_size,
        use_requant,
    )
    ...
    for i in pbar:
        ...
        if world_size > 1:
            _broadcast_parameters_contiguous(layer, src=0, group=group)
