# coding=utf-8
"""
Weight quantization entry for GPTQ with data-parallel Hessian accumulation.

Use with ``torchrun --nproc_per_node=R`` and ``ptq_gptq_dp.py``.
"""
...
from gptq_utils.gptq_dp_utils import gptq_fwrd_data_parallel, resolve_local_cuda_device
from utils import data_utils, dist_utils, model_utils


def quantize_weights_gptq_dp(args, analyzer: model_utils.ModelAnalyzer):
    """
    Same contract as ``gptq_utils.main.quantize_weights`` for ``w_method=gptq``,
    but calibration uses ``gptq_fwrd_data_parallel``. Checkpoint is written only on
    global rank 0.
    """
    ...
    if args.load_qmodel_path:
        ...
        quantizers = gptq_fwrd_data_parallel(args, analyzer, trainloader, dev)
