# coding=utf-8
"""
PTQ driver for base GPTAQ with data-parallel Hessian / dXXT (multi-GPU).

Launch example::

    torchrun --nnodes=1 --nproc_per_node=4 --rdzv_endpoint=localhost:29500 \\
        ptq_gptaq_dp.py --model ... --w_method gptaq ...

``--w_method`` must stay ``gptaq`` (same checkpoints as single-GPU path).
"""
...
def main(args):
    ...
    if args.w_method != "gptaq":
        logging.warning(
            "ptq_gptaq_dp is intended for w_method=gptaq; got %s",
            args.w_method,
        )
    ...
    model = quantize_weights_gptaq_dp(args, analyzer)
