    ...
    def fake_quantize(self, x, st_idx=None, end_idx=None):
        ...
        if self.ready() and self.bits < 16:
            ...
            if st_idx is not None and end_idx is not None:
                ...
                zero = zero[st_idx:end_idx]
            if self.sym:
                mq = self.maxq.to(device=x.device, dtype=x.dtype)
                qint = torch.clamp(torch.round(x / scale), -(mq + 1), mq)
                return (sym_dequant(qint, scale)).to(x_dtype), qint, scale
            mq = self.maxq.to(device=x.device, dtype=x.dtype)
            qint = torch.clamp(torch.round(x / scale) + zero, 0, mq)
            return (asym_dequant(qint, scale, zero)).to(x_dtype), qint, scale
        else:
