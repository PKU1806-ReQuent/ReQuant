# coding=utf-8
"""
PTQ driver for AWQ with data-parallel activation-stat collection (multi-GPU).

Launch example::

    torchrun --nnodes=1 --nproc_per_node=4 --rdzv_endpoint=localhost:29500 \
        ptq_awq_dp.py --model ... --w_method awq ...

``--w_method`` must stay ``awq``.
"""
...
from utils import data_utils, dist_utils, eval_utils, hadamard_utils, memory_utils, \
                  model_utils, quant_utils, rotation_utils
...
def main(args):
    ...
    if args.w_method != "awq":
        logging.warning(
            "ptq_awq_dp is intended for w_method=awq; got %s",
            args.w_method,
        )
    ...
    model = quantize_weights_awq_dp(args, analyzer)
